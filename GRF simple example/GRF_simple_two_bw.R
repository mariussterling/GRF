...
set.seed(100)
node_sizes = c(1,2,3,5,7,10,15) #bias controlling param of RFS
#node_sizes = c(1)
node_size_2 = 5
...
for (node_size in node_sizes){
  ...
  theta_true_test =  0 #theta_triangle(X_test, width)# + qnorm(tau)*sig
  grids = nrow(X_test)
  ...
  w_test = weights_forest(rf_2,X_test)
  e_multipliers = lapply(1:reps, function(j) lapply(1:b, function(j) rnorm(n, 0, 1)))
  T_stat = lapply(1:reps, function(k) sapply(1:b, function(j)
    (-(H_hat[,k]^(-1/2)*w_test[[k]]*psi[[k]])%*% e_multipliers[[k]][[j]])@x))
  T_stat_abs = lapply(1:reps, function(j) abs((T_stat[[j]])))
  ...
  coverages_widths_PW[[as.character(node_size)]] = coverage_true
