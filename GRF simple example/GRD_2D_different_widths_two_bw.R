...
set.seed(100)
node_sizes = c(2,3,5,7,10,15) #bias controlling param of RFS
#node_size_1 = 5 #bias controlling param of RFS
node_size_2 = 3  
...
for (node_size_1 in node_sizes){
  ...
  ## Fitting the forest ----
  rf_1= fit_forest(X,Y,tau = tau,node_size = node_size_1)
  theta_hat_test = predict_forest(rf_1,X_test)
  w_test = weights_forest(rf_1,X_test)
  #mat <- do.call("cbind",theta_hat_test)
  #theta_hat_expected = rowMeans(mat)
  
  rf_2= fit_forest(X,Y,tau = tau,node_size = node_size_2)
  w_test_2 = weights_forest(rf_2,X_test)
  ...
  T_stat = lapply(1:reps, function(k) sapply(1:b, function(j)
    (-(H_hat[,k]^(-1/2)*w_test_2[[k]]*psi[[k]])%*% e_multipliers[[k]][[j]])@x))
...
#----
coverages_widths_PW[[as.character(node_size_1)]] = coverage_true
coverages_widths_uniform[[as.character(node_size_1)]] = coverage_true_uniform
}
