...
## Confidence interval with test stat ----
  ...
  #data_std = scale(Y) #standardize the data
  q_norm = qnorm(1-alpha_sig)
  #q_norm = lapply(1:reps, function(j) colQuantiles(dnorm(Y),  probs= c(1-alpha_sig)))
...
#[,seq(1, nrow(X), length.out = grids) ] #t stats for CB
